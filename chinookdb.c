 ...
 * 
 * This code was some of the most gruesome I've ever seen.  Honestly. 
 * It's bad enough that it was K&R, rather than ANSI C (I don't think I've
 * seen any K&R C code in the last 15 years, and I haven't written any in 
 * the last 20) but it also entangled a number of things:
 *
 * It use to use "long" in many places.  In reality, it's pretty much 
 * obvious from the code that nearly all the occurrences of long are really
 * better replaced with uint32_t.  So that's what I did.  Cautiously.  I 
 * bet that I missed some places.   
 *
 * It also uses lseek() with longs, which should really be off_t in any 
 * reasonably modern system.
 *
 * This code will undoubtably need some additional testing after this 
 * major overhaul.
 * 
 * 							-- markv
 */
 ...
 * the easiest thing in the world, but there you go...
 *
 *    Chinook's		Milhouse
 *      BLACK	 	
 *   7  15  23  31	   0   1   2   3
 * 3  11  19  27	 4   5   6   7 
 *   6  14  22  30	   8   9  10  11
 * 2  10  18  26	12  13  14  15
 *   5  13  21  29	  16  17  18  19
 * 1   9  17  25	20  21  22  23 
 *   4  12  20  28	  24  25  26  27
