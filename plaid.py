...
def moves(n):
    ...
    for dx, dy in [(-1, 1), (1, 1), (-1, -1), (1, -1)]:
        try:
            n2 = xyton(x + dx, y + dy)
            l.append((n, n2))
        except Illegal as msg:
            continue
    return l


validmoves = reduce(operator.__add__, [moves(x) for x in range(32)], [])
validmoves.sort()

fwdmoves = [x for x in validmoves if x[1] > x[0]]
revmoves = [x for x in validmoves if x[0] > x[1]]
...
validjumps = reduce(operator.__add__, [jumps(x) for x in range(32)], [])
validjumps.sort()

print("%d validjumps" % len(validjumps), file=sys.stderr)

fwdjumps = [x for x in validjumps if x[2] > x[0]]
revjumps = [x for x in validjumps if x[0] > x[2]]
...
def genman(name, f, fwdmoves, kinging):
    print(
        "CheckerBoard *\n%smanmove(int c, CheckerBoard *b, CheckerBoard *m)"
        % name
    )
    print("{")
    print("    switch (c) {")
...
genman("red", "R", fwdmoves, list(range(28, 32)))
genman("white", "W", revmoves, list(range(0, 4)))


def genking(name, f, moves):
    print(
        "CheckerBoard *\n%skingmove(int c, CheckerBoard *b, CheckerBoard *m)"
        % name
    )
    print("{")
    print("    switch (c) {")
...
def genmanresolve(name, f, g, jumps, kinging):
    print(
        "CheckerBoard *\n%smanresolve(int c, CheckerBoard *b, CheckerBoard *m)"
        % name
    )
    print("{")
    ...
    for n in range(32):
        ...
        for n1, n2, n3 in jumps:
            if n == n1:
                if n3 in kinging:
                    print(
                        "\tif (UNOCC(%d) && OCC(%s, %d)) FINISHMAN(%s, %s, %s, %d, %d, %d)"
                        % (n3, g, n2, name, f, g, n1, n2, n3)
                    )
                else:
                    print(
                        "\tif (UNOCC(%d) && OCC(%s, %d)) JUMPMAN(%s, %s, %s, %d, %d, %d)"
                        % (n3, g, n2, name, f, g, n1, n2, n3)
                    )
...
def genmanjump(name, f, g, jumps, kinging):
    print(
        "CheckerBoard *\n%smanjump(int c, CheckerBoard *b, CheckerBoard *m)"
        % name
    )
    print("{")
    print("    CheckerBoard tmp ;")
...
def genkingresolve(name, f, g, jumps):
    print(
        "CheckerBoard *\n%skingresolve(int c, CheckerBoard *b, CheckerBoard *m)"
        % name
    )
    print("{")
    print("    CheckerBoard tmp ;")
    ...
    for n in range(32):
        ...
        for n1, n2, n3 in jumps:
            if n == n1:
                print(
                    "\tif (UNOCC(%d) && OCC(%s, %d)) JUMPKING(%s, %s, %s, %d, %d, %d)"
                    % (n3, g, n2, name, f, g, n1, n2, n3)
                )
        print("\tif (m == n) { *m = *b ; m->hash ^= COLORSWAP; m++ ;}")
        print("\tbreak ;")
...
def genkingjump(name, f, g, jumps):
    print(
        "CheckerBoard *\n%skingjump(int c, CheckerBoard *b, CheckerBoard *m)"
        % name
    )
    print("{")
    print("    CheckerBoard tmp ;")
...
genmanresolve("red", "R", "W", fwdjumps, list(range(28, 32)))
genmanjump("red", "R", "W", fwdjumps, list(range(28, 32)))
genmanresolve("white", "W", "R", revjumps, list(range(0, 4)))
genmanjump("white", "W", "R", revjumps, list(range(0, 4)))
