...
int db_init(int suggestedMB, char out[256])
			...
			for(bk=0;bk<=nb;bk++)
				...
				for(wk=0;wk<=nw;wk++)
					...
					if(bm+bk==wm+wk && wk>bk)
						continue;
					sprintf(dbname,"cake-db/db%i_%i%i%i%i.cpr",bm+bk+wm+wk,bm,bk,wm,wk);
		...
		for(nb=maxpieces-maxpiece;nb<=maxpiece;nb++)
			...
			for(bk=0;bk<=nb;bk++)
				...
				for(wk=0;wk<=nw;wk++)
					...
					sprintf(dbname,"cake-db/db%i_%i%i%i%i.idx",bm+bk+wm+wk,bm,bk,wm,wk);
					sprintf(out,"parsing %s",dbname);
					fp = fopen(dbname,"rb");
					if(fp != NULL)
						{
	...
	if(blockinfo == NULL && memsize!=0)
		{	
		sprintf(str,"\ncould not allocate LRU list (%ld KB)",(cachesize)*sizeof(struct bi)/1024);
		//logtofile(str);
		//error = GetLastError();
		sprintf(str,"\nerror code %i",errno);
		//logtofile(str);
	...
	sprintf(str,"\nallocated %ld KB for LRU linked list",(cachesize)*sizeof(struct bi)/1024);
	//logtofile(str);
